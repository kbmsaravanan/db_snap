# Add schema compare source to destination add missing
# drop schema compare destination to source drop missing
import main as sh
import json
import shutil
import os

DEST_CONFIG_SECTION = "dest_database"
...
def compare_src_to_dest():
    ...
    dest_snapshot = sh.get_snapshot(DEST_CONFIG_SECTION)
    ...
    migrate_str += compare_tables(src_snapshot.tables,
                                  dest_snapshot.tables)
    migrate_str += compare_function(src_snapshot.functions,
                                    dest_snapshot.functions)
...
def compare_function(src_functions, dest_functions):
    ...
    for src_function in src_functions:
        dest_function = list(
            filter(lambda x: x.function_name == src_function.function_name, dest_functions))
    ...
    for dest_function in dest_functions:
        function_str += "DROP function {function_name}; \n".format(
...
def compare_tables(src_tables, dest_tables):
    ...
    for src_table in src_tables:
        dest_table = list(filter(lambda x: x.table_name ==
                                 src_table.table_name, dest_tables))
        if not dest_table:
    ...
    for dest_table in dest_tables:
        drop_str = "DROP TABLE {table_name}; \n".format(
            table_name=dest_table.table_name)
        table_str += drop_str
    return table_str
...
def compare_columns(table_name, src_columns, dest_columns):
    ...
    for src_col in src_columns:
        dest_col = list(filter(lambda x: x.column_name ==
                               src_col.column_name, dest_columns))
        if not dest_col:
        ...
        elif src_col.numeric_precision != dest_col[0].numeric_precision or src_col.numeric_scale != dest_col[0].numeric_scale:
            alter_str += alter_column_length(table_name, src_col)
...
def get_column_type(column):
    if column.data_type == 'character varying' and column.character_maximum_length is None:
    ...
    if column.data_type == 'character varying':
        ...
        return 'numeric({0},{1})'.format(int(column.numeric_precision), int(column.numeric_scale))
    else:
...
def alter_add_column(table_name, column):
    ...
    if column.column_default is not None and column.column_default.startswith('nextval('):
        data_type = get_serial_name(column.data_type)
        default = ''
    elif column.column_default is not None:
        default = 'DEFAULT ' + default
    return "ALTER TABLE {table_name} ADD COLUMN {column_name} {data_type} {nullable} {default};\n" \
...
def alter_column_type(table_name, column):
    column_name = column.column_name
    data_type = get_column_type(column)
    return "ALTER TABLE {table_name} ALTER COLUMN {column_name} TYPE {data_type};\n" \
        .format(table_name=table_name,
                column_name=column_name,
                data_type=data_type)
...
def alter_column_null(table_name, column):
    column_name = column.column_name
    is_nullable = 'DROP NOT NULL' if (
        column.is_nullable == 'YES') else 'SET NOT NULL'
    return "ALTER TABLE {table_name} ALTER COLUMN {column_name} {is_nullable};\n" \
        .format(table_name=table_name,
                column_name=column_name,
...
def alter_column_default(table_name, column):
    column_name = column.column_name
    default = 'DROP DEFAULT' if (
        column.column_default is None) else 'SET DEFAULT ' + column.column_default
    return "ALTER TABLE {table_name} ALTER COLUMN {column_name} {default};\n" \
        .format(table_name=table_name,
                column_name=column_name,
                default=default)
