...
SRC_CONFIG_SECTION = "src_database"
...
def create_table_column(column, cname_padding, type_padding):
    ...
    return COLUMN.format(column_name=to_case_sensitive(column_name).ljust(cname_padding),
...
def create_table_schema(schema, table):
    ...
    max_col_length = len(
        max(list(map(lambda x: x.column_name, column_list)), key=len))
    max_type_length = len(
        max(list(map(lambda x: x.data_type, column_list)), key=len))
    for column in column_list:
        column_script += create_table_column(column,
                                             max_col_length, max_type_length)
    key_script = get_table_key_constraints(table.constraints)
    index_script = get_table_indexes(table.indexes)
    table_script = CREATE_TABLE.format(schema=schema,
                                       table_name=to_case_sensitive(
                                           table.table_name),
...
def create_schema_script():
    sh.generate_snap(SRC_CONFIG_SECTION)
    ...
    for table in snapshot.tables:
        ...
        table_file = open(r'{0}\tables\{1}.sql'.format(
            snapshot.schema, table.table_name), 'w')
        table_file.write(table_script)

    for function in snapshot.functions:
        function_file = open(r'{0}\functions\{1}.sql'.format(
            snapshot.schema, function.function_name), 'w')
